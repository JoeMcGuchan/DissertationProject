...
\begin{tikzpicture}
\begin{axis} [width=\textwidth, height=\axisdefaultheight] 
\addplot table [x=Time (minutes), y=Time of Signal (milliseconds), col sep=comma] {./Data/LondonToBeijing.csv};
\end{axis}
\end{tikzpicture}
\end{figure}
...
In both the london to Beijng and London to Beijing routes , to examine this further, I ran a separate test (not in the model) at an increased speed. In the increased speed you can see a periodicity, I looked at this data more closely and found the peacks to be very close to 60 minutes apart. I suspected that this might be caused by the rotation of the earth underneath the grid, as there are 24 orbital planes in the starlink constellation, so I ran the test one more time with the earth static, %TODO
...
\begin{tikzpicture}
\begin{axis} [width=\textwidth, height=\axisdefaultheight] 
\addplot table [x=Time (minutes), y=Time of Signal (milliseconds), col sep=comma] {./Data/LondonToNewYorkAccelerated.csv};
\end{axis}
\end{tikzpicture}
\begin{tikzpicture}
\begin{axis} [width=\textwidth, height=\axisdefaultheight] 
\addplot table [x=Time (Minutes), y=Time of Signal (milliseconds), col sep=comma] {./Data/LondonToBeijingAccelerated.csv};
\end{axis}
\end{tikzpicture}
\end{figure}
